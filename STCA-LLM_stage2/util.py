...
def train(args, Dtr, Val):
    adj  = pd.read_csv(r'D:\pythonproject\STCA-LLM\STCA-LLM_stage2\dataset\DTW_normalized.csv',header=None)
    adj = np.array(adj)
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        if epoch + 1 >= min_epochs and val_loss < min_val_loss:
            ...
            model.gpt.gpt2.save_pretrained(r'D:\pythonproject\STCA-LLM\gptweights\gpt2_weights_stage2')
            torch.save({'model': model.state_dict()}, r'D:\pythonproject\STCA-LLM\STCA-LLM_stage2\models\gpt.pkl')
...
def test(args, Dte, scaler):
   ...
   # Transpose after checking lengths
    ...
    #pd.DataFrame(ys.T).to_csv(r'D:\pythonproject\STCA-LLM\STCA-LLM_stage2\results\ys.csv', index=False)
    #pd.DataFrame(preds.T).to_csv(r'D:\pythonproject\STCA-LLM\STCA-LLM_stage2\results\preds.csv', index=False) 
    ...
    df.to_csv(r'D:\pythonproject\STCA-LLM\STCA-LLM_stage2\results\result.csv', index=False)
    plt.show()
