...
def train(args, Dtr, Val):
    adj  = pd.read_csv(r'D:\pythonproject\STCA-LLM\STCA-LLM_stage1\dataset\DTW_normalized.csv',header=None)
    adj = np.array(adj)
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        if epoch + 1 >= min_epochs and val_loss < min_val_loss:
            ...
            model.gpt.gpt2.save_pretrained(r'D:\pythonproject\STCA-LLM\gptweights\gpt2_weights_stage1')
            torch.save({'model': model.state_dict()}, r'D:\pythonproject\STCA-LLM\STCA-LLM_stage1\models\gpt.pkl')
    ...
    #state = {'model': best_model.state_dict()}
    #torch.save(state, r'D:\pythonproject\stcllm\models\gpt.pkl')
    #model.gpt.gpt2.save_pretrained(r'D:\pythonproject\stcallm - 副本\gptweights\gpt2_weights_stage1')
    torch.save({'model': model.state_dict()}, r'D:\pythonproject\STCA-LLM\STCA-LLM_stage1\models\gpt.pkl')
...
def test(args, Dte, scaler):
    ...
    adj = np.array(adj)
    model = stcllm(args, device, adj).to(device)
    model.load_state_dict(torch.load(r'D:\pythonproject\STCA-LLM\STCA-LLM_stage1\models\gpt.pkl')['model'])
    model.eval()
    print('predicting...')
    ...
    preds = np.array(preds).T  # Transpose after checking lengths).T  # Transpose after checking lengths
    ...
    df.to_csv(r'D:\pythonproject\STCA-LLM\STCA-LLM_stage1\results\result.csv', index=False)
    plt.show()


def plot(y, pred, ind, label):
    # plot
    plt.plot(y[:500], color='red', label='pred value')

    plt.plot(pred[:500], color='blue', label='true value')
